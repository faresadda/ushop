...
const checkUserPhone = require("../middlewares/phone");
const {user} = require("../middlewares/upload")
...
router
  .route("/addphone/:id")
  .put(authorizationUser, phoneValidator, usersService.addPhone);

router
  .route("/addaddress/:id")
  .put(authorizationUser, addressValidator, usersService.addAddress);

// New route for getting all users (admin only)
router
  .route("/users")
  .get(authorizationUser, authorizationAdmin, usersService.getUsers);
router
  .route("/admins")
  .get(authorizationUser, authorizationAdmin, usersService.getAdmins);
