...
import { MdOutlineDiscount, MdOutlineReportGmailerrorred } from "react-icons/md";
...
export default function Products() {
  ...
  const removeFromFavorites = (e, product) => {
    e.stopPropagation();
    setFavorites(() => {
      const favoritesArray = favorites.filter((favorite) => favorite.id !== product.id);
      localStorage.setItem("favorites", JSON.stringify(favoritesArray));
      return favoritesArray;
    });
        ...
        <div className="flex gap-4 overflow-x-auto pb-4 px-1 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent">
          <p
            ...
            className={`flex flex-col items-center min-w-[min-content] bg-primary ${
              selectedCategory === "All Products" ? "border-secondary" : "border-transparent"
          ...
          {categories.map((cat) => (
            <div
              ...
              className={`flex flex-col items-center min-w-[min-content] bg-primary ${
                selectedCategory === cat.title ? "border-secondary " : "border-transparent"
            ...
            >
              <div className="w-14 h-14 md:w-16 md:h-16 flex items-center justify-center mb-2">
                <img src={cat.img} alt={cat.title} className="max-h-full max-w-full object-contain" />
              </div>
              <span className="text-xs md:text-sm text-gray-700 text-center font-medium">{cat.title}</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-8">
          {products.length === 0 ? (
            <div className="text-lg text-red-500 my-20 flex justify-center items-center gap-2 py-10">
              Products not found <MdOutlineReportGmailerrorred className="text-2xl" />
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                {currentProducts.map((product, index) => (
                  <div
                    ...
                    onClick={() => {
                      setIndex(indexOfFirstProduct + index);
                      navigate('/shop');
                    }}
                  ...
                  >
                    <div className="relative overflow-hidden rounded-xl">
                      ...
                      <button
                        className={`absolute left-2 top-2 p-2 bg-white text-red-500 rounded-full shadow-md z-10 hover:bg-gray-100 transition-colors ${
                          favorites.find(favorite => favorite.id === product.id) ? "animate-heart" : ""
                        }`}
                      >
                        {favorites.find(favorite => favorite.id === product.id) ? (
                          <AiFillHeart className="text-xl" onClick={(e) => removeFromFavorites(e, product)} />
                        ) : (
                          <AiOutlineHeart className="text-xl" onClick={(e) => AddToFavorites(e, product)} />
                        )}
                      </button>
                    ...
                    <div className="mt-3 flex flex-col gap-1">
                      ...
                      <div className="flex sm:items-center gap-1 text-gray-600 flex-col sm:flex-row">
                        ...
                        <p className="text-xs">4.5 | {product.sold} sold</p>
                      </div>
                  ...
                  <div className="flex mx-2">
                    {[...Array(totalPages)].map((_, i) => {
                      ...
                      if (
                        ...
                        return (
                          <span key={pageNumber} className="w-10 h-10 mx-1 flex items-center justify-center">
                            ...
                          </span>
                        );
                      }
